 ...
 */
var typelib = require('./dev').require('hyperloop-common').compiler.type,
...
iOSType.prototype._parse = function(metabase) {
	...
	// check to see if a constant
	var symbol = metabase.symbols && metabase.symbols[type];
	if (symbol && (symbol.metatype == 'constant' || symbol.metatype == 'variable')) {
		this._const = (constRegex.test(symbol.type) && 'const') || '';
		this._metatype = SuperClass.NATIVE_CONSTANT;
		this._type  = symbol.type.replace('const','').trim();
		this._name = this._type;
		this._value = this._type;
		var subtype = typelib.resolveType(this._type);
		if (subtype._jstype !== SuperClass.JS_UNDEFINED) {
			this._jstype = subtype._jstype;
			this._nativetype = subtype._nativetype;
		}
		return;
	}
...
iOSType.prototype.toNativeBody = function(varname, preamble, cleanup, declare) {
	if (this.isNativeString()) {
		...
		var subvar = this.makeSafeVarname(varname)+'buf';
		// char accepts char code or boolean
		preamble.push('char * '+subvar+' = nullptr;');
		preamble.push('if (JSValueIsNumber(ctx,'+varname+'))');
		preamble.push('{');
		preamble.push('\t'+subvar+' = new char[1];');
		preamble.push('\t'+subvar+'[0] = static_cast<char>(JSValueToNumber(ctx,'+varname+',exception));');
		preamble.push('}');
		preamble.push('else if (JSValueIsBoolean(ctx,'+varname+'))');
		preamble.push('{');
		preamble.push('\t'+subvar+' = new char[1];');
		preamble.push('\t'+subvar+'[0] = static_cast<char>(JSValueToBoolean(ctx,'+varname+') ? 1 : 0);');
		preamble.push('}');
		preamble.push('else if (JSValueIsObject(ctx,'+varname+'))');
		preamble.push('{');
		preamble.push('\t'+subvar+' = static_cast<char*>(HyperloopJSValueToVoidPointer(ctx,JSValueToObject(ctx,'+varname+',exception),exception));');
		preamble.push('}');

		preamble.push('if ('+subvar+' == nullptr)');
		preamble.push('{');
		preamble.push('\t'+subvar+' = HyperloopJSValueToStringCopy(ctx,'+varname+',exception);');
		preamble.push('}');

		cleanup.push('delete [] '+subvar+';');

		if (this._length===1) {
			return '*[[NSString stringWithFormat:@"%c",'+subvar+'[0]] UTF8String]';
		}
		return '[[NSString stringWithUTF8String:'+subvar+'] UTF8String]';
	}
