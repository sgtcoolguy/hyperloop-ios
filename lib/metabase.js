...
function execClang(file, opts, callback) {
	...
	buildlib.getXcodeSettings(function(err, settings) {
		...
		var args = [].concat(
				util.format('-Xclang -ast-dump -miphoneos-version-min=%s -isysroot %s -arch %s -x objective-c --analyze -fno-color-diagnostics', opts.minVersion, isysroot, arch).split(/\s+/),
				file
			);
