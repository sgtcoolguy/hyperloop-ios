 ...
 */
function generateMethodSelector(options,metabase,state,arguments,typelib,instance,code,cleanup) {
	var selector = '',
		start = instance ? 1 : 0;

	arguments.forEach(function(m,i){
		var type = m.type,
			value = m.name+'$'+i,
...
function generateFunction(options, metabase, state, indent, fnname, fn) {
	...
	var selector = generateMethodSelector(options,metabase,state,fn.arguments,typelib,false,code,cleanup);

	if (typeobj.isNativeStruct()) {
		code.push(typeobj.toName()+' '+varname+'$ = '+fnname+'('+selector+');');
		code.push(typeobj.toCast()+' '+varname+' = ('+typeobj.toCast()+')malloc(sizeof('+typeobj.toName()+'));');
		code.push('memcpy('+varname+',&'+varname+'$,'+'sizeof('+typeobj.toName()+'));');
