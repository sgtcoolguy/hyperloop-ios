 ...
 */
function generateMethodArguments(options,metabase,state,args,typelib,instance,code,cleanup) {
	var selector = '',
		start = instance ? 1 : 0;
...
function generateMethodSelector(options,metabase,state,indent,typelib,code,cleanup,method,instance) {
	...
	if (method.formatter) {
		...
		code.push(indent+'if (JSValueIsString(ctx,argValue))');
		code.push(indent+'{');
		...
		code.push(indent+'}');
		code.push(indent+'else if (JSValueIsObject(ctx,argValue))');
		code.push(indent+'{');
		code.push(indent+indent+'arg = nil; //TODO');
		code.push(indent+'}');
...
function generateFunction(options, metabase, state, indent, fnname, fn) {
	...
	var selector = generateMethodArguments(options,metabase,state,fn.arguments,typelib,false,code,cleanup);
