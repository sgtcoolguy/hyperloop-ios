...
function getArchitectures (options, callback) {
	buildlib.getXcodeSettings(function(err,settings){
		...
		if (options.arch) {
			...
			if (archs.length===0) {
				log.fatal("at least one architecture must be specified using --arch");
			}
...
function prepareClass(options, metabase, state, classname, code) {
	...
	if (!entry) {
		log.fatal("Couldn't find class named:",classname.red);
	}

	//FIXME we need to move this before we are ready to compile
	if (!available(entry,options.minVersion)) {
		log.fatal(classname.green.bold+" is not available for this version of ios");
	}
...
function prepareFunction(options, metabase, state, fnname, code) {
	...
	if (!available(entry,options.minVersion)) {
		log.fatal("function named:",fnname,"not available for this version of ios");
	}
...
function prepareType(options, metabase, state, type) {
	...
	if (!available(entry,options.minVersion)) {
		log.fatal("type named:",type,"not available for this version of ios");
	}

	if (entry.vector) {
		log.fatal("vector type named:",type,"not supported");
	}

	if (type=='vImage_CGAffineTransform') {
		log.fatal("type named:",type,"not supported");
	}
...
function generateMethod(options, metabase, state, indent, varname, classname, method, methodname) {
	...
	method.args.forEach(function(m,i){
		var type = m.subtype,
			...
			typeobj = typelib.resolveType(metabase, type), 
	...
	var methodBlock = '['+varname+' '+method.name+''+selector.trim()+']',
		...
		methodType = typelib.resolveType(metabase,method.returnSubtype);
	...
	if (methodType.isNativeVoid() && !methodType.isPointer()) {
		...
		var rpreamble = [],
			declare = [],
			classObj = typelib.resolveType(metabase,classname),
...
function generateGetterProperty(options, metabase, state, library, classname, propertyname, property, varname, cast, indent) {
	...
	var code = [],
		...
		typeobj = typelib.resolveType(metabase, property.subtype),
		preamble = [],
		cleanup = [],
		declare = [],
