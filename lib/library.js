...
function generateFunction(options, metabase, state, indent, fnname, fn) {
	var code = [],
		...
		varname = typeobj.toName().toLowerCase(),
		cleanup = [];

	if (!typeobj.isNativeVoid()) {
		addExtern(state, 'JSValueRef '+typeobj.toJSValueName()+'(JSContextRef,'+typeobj.toCast()+',JSValueRef*);');
		returnBlock = '';
	}

	var selector = generateMethodArguments(options,metabase,state,fn,typelib,false,code,cleanup,indent);

	if (typeobj.isNativeStruct()) {
		typeobj.toAllocStruct(varname, fnname, selector, code);
	} else if (typeobj.isNativeVoid()) {
		code.push(fnname+'('+selector+');');
	} else {
		code.push(typeobj.toCast()+' '+varname+' = '+fnname+'('+selector+');');
	}

	if (typeobj.isNativeVoid()) {
		code.push('return JSValueMakeUndefined(ctx);');
	} else {
		code.push('return '+typeobj.toJSValueName()+'(ctx,'+varname+',exception);');
	}
...
function generateNewInstance(state, metabase, indent, classname, cast, varname, methodnode) {
	...
	if (methodnode.selector) {
		var selector = generateMethodSelector(state.options,state.metabase,state,indent,typelib,code,cleanup,method,false);
...
function generateCustomClass(options, state, metabase, classname, filename) {
	...
	classinfo.methods && Object.keys(classinfo.methods).forEach(function(name,methodIndex){
		...
		methods.forEach(function(method){
			...
			var actionName = '__'+name+'_Action';
			...
			argTypes.forEach(function(type,index){
				var typecode = type.toJSBody(argNames[index],preamble,cleanup,externs);
			...
			if (returnType.isNativeVoid()) {
				...
				preamble = [];
				cleanup = [];		
				var resultCode = returnType.toNativeBody('result$',preamble,cleanup,externs);
				preamble.forEach(function(c){
					implementation_code.push(indent+c);
				});
	...
	options.frameworks.forEach(function(imp){
		code.push('#import <'+imp+'/'+imp+'.h>');
	});
