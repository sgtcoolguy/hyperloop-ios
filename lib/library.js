...
exports.isMethodConstructor = isMethodConstructor;
...
exports.prepareIncludes = prepareIncludes;
...
function defaultFrameworks () {
	return ['Foundation','JavaScriptCore'];
}
...
function compileLibrary (opts, arch, metabase, callback) {
	...
	prepareFrameworksOption(options);
	...
	if (options.srcfiles.length) {
		...
		if(options.thirdPartyFrameworks) {
			options.thirdPartyFrameworks.forEach(function(f) {
				options.cflags.push('-F' + path.resolve(f.directory, '..', '..'));
			});
		}
		options.outdir = path.join(options.dest,'build');
		options.linkflags = options.linkflags || [];
		options.linkflags = options.linkflags.concat(
			(options.frameworks||defaultFrameworks()).map(function(f){return '-framework '+f})
		);
...
function prepareFrameworksOption(options) {
	if (!options.frameworks) {
		options.frameworks = [];
	}
	if (typeof(options.frameworks) == 'string') {
 ...
 */
function generateMethod(options, metabase, state, indent, varname, classname, method, methodname, superCall, isConstructor) {
	var code = [],
		cleanup = [];
	var selector = generateMethodSelector(options,metabase,state,indent,typelib,code,cleanup,method,method.instance && !isConstructor);
	var methodBlock = '',
		returnBlock = 'return JSValueMakeUndefined(ctx);',
		methodType = typelib.resolveType(method.returnSubtype);

	if (superCall) {
		methodBlock = '['+varname+' super_'+method.name+''+selector.trim()+']';
	} else if (isConstructor) {
		methodBlock = '[[['+classname+' alloc] '+method.name+''+selector.trim()+'] autorelease]';
	} else {
...
/**
 * return true if the method is treated as an instance method (initWithXXX)
 */
function isMethodConstructor (options, metabase, state, method) {
	log.debug(method);
	return method.name.match(/^initWith/) != null;
}
...
/*
 * import all known headers.
 * TODO should narrow down more basead on class dependency
 */
function prepareIncludes(options,metabase,state,code,skip_state) {
	prepareImportOption(options);
	!skip_state && state.imports && Object.keys(state.imports).forEach(function(name){
		var imp = state.imports[name],
			quote = imp && imp.charAt(0)!='<' && imp.charAt(0)!='"',
			line = quote ? ('"'+imp+'"') : imp;
		if (line && options.imports.indexOf(line)<0) {
			options.imports.push(line);
		}
	});
	...
	options.frameworks && options.frameworks.forEach(function(f) {
		...
		if (imp && options.imports.indexOf(imp) < 0) {
			options.imports.push(imp);
		}
	});

	generateImports(options, code);
}
...
function generateCheckNSException(code, indent) {
	code.push(indent+'if (_exception != NULL && !JSValueIsNull(ctx, _exception)) {');
...
function generateImports(options,code) {
	prepareImportOption(options);
	code.push('');
...
function generateCustomClass(options, state, metabase, classname, filename) {
	...
	state.imports[classname] = '<'+classname+'.h>';
	...
	if (state.supermethods && state.supermethods[classname]) {
		Object.keys(state.supermethods[classname]).forEach(function(key) {
			...
			var method = _.clone(m.method);
			method.superCall = true;
			classinfo.methods[m.name].push(method);
		});					
	}
	...
	classinfo.methods && Object.keys(classinfo.methods).forEach(function(name,methodIndex){
		...
		methods.forEach(function(method){
			var returnTypeOrig = method.returnTypeOrig ? method.returnTypeOrig : method.returnType;
			var returnType = typelib.resolveType(returnTypeOrig),
			...
			for (var c=0;c<method.args.length;c++) {
				...
				if (argSelector.length == 0) {
					argSelector.push(arg.name);
				} else {
					argSelector.push(arg.name+':'+arg.name);
				}
			}

			var methodSelector = (method.instance ? '-' : '+')+'('+returnTypeOrig+')'+name+(args.length?':':''),
				argNames = [],
				preamble = [], 
				cleanup = [];
			...
			if (method.action) {
				...
				if (arg.length == 0) {
					implementation_code.push('\t'+returnBlock+'[super '+method.name+'];');
				} else {
					implementation_code.push('\t'+returnBlock+'[super '+method.name+':'+argSelector.join(' ')+'];');
				}
