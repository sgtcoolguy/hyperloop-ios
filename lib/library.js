...
function generateFunction(options, metabase, state, indent, fnname, fn) {
	var code = [],
		returnType = typelib.resolveType(fn.returnType),
		varname = returnType.toName().toLowerCase(),
		cleanup = [];

	addExtern(state, 'JSValueRef '+returnType.toJSValueName()+'(JSContextRef,'+returnType.toName()+',JSValueRef*);');

	var selector = generateMethodSelector(options,metabase,state,fn.arguments,typelib,false,code,cleanup);

	code.push(returnType.toName()+' '+varname+' = '+fnname+'('+selector+');'),
	code.push('return '+returnType.toJSValueName()+'(ctx,'+varname+',exception);');
