...
function findProperty (metabase, classname, property) {
	...
	if (cls) {
		var p = cls.properties && cls.properties[property];
		if (p) {
			return p;
		}
...
function findMethod (metabase, classname, methodname) {
	var cls = metabase.classes[classname] || metabase.protocols[classname];
	if (cls) {
	...
	Object.keys(symbols).forEach(function(key){
		...
		switch (entry.type) {
			case 'function': {
				...
				if (!fn) {
					nodefail(entry.location, "couldn't find function named: "+entry.name.yellow);
				}
				if (entry.function.formatter) {
					...
					if (fn.arguments.length!==entry.argcount) {
						nodefail(entry.location, "wrong number of arguments passed to "+entry.name.yellow+". expected "+fn.arguments.length+" but received "+entry.argcount);
					}
				}
				break;
			}
			...
			case 'statement': {
				var sym = metabase.classes[entry.class] || metabase.protocols[entry.class];
				if (!sym) {
				...
				switch (entry.metatype) {
					case 'setter': {
						if (property.attributes.indexOf('readwrite')==-1) {
...
function defineMethod(options,state,arch,node,dict,fail) {
	...
	if (!vardef || !vardef['class']) {
		throw new Error('failed to lookup the definition of '+varname);
	}
	...
	var classname = vardef['class'],
		...
		methods = entry.methods[method] || entry.methods[method];
	...
	var signature = callstr.substr(method.length+1).replace(/\:$/,''),
		methodname = method,
		fn = jsgen.generateMethodName(classname, methodname)+'_'+signature,
		methodObj = _.find(entry.methods[methodname], function(m) {return m.selector==callstr;});
