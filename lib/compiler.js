...
function constructorFail(node, cls, nodefail) {
	var msg = node.args.length===0 ? 'no argument' :
		((node.args.length+' arg')+((node.args.length>1) ? 's':''));
	nodefail(node, "couldn't find constructor for class: "+cls.yellow+" and "+msg.yellow);
}
...
function getInstanceMethodSymbol(state, cls, method, varname, symbolname, node, nodefail) {
	...
	return {type:'method',metatype:'instance',symbolname:symbolname,instance:varname,class:cls,name:method,location:node.start,argcount:node.args.length,method:m,returnType:m.returnType,returnTypeOrig:m.returnTypeOrig};
}
...
function getConstructorSymbol(state, cls, node, nodefail) {
	var m = findConstructor(state.metabase, cls, node.args, node, nodefail),
		symbolname;
	if (!m) {
		constructorFail(node, cls, nodefail);
	}
...
function findType (metabase, name) {
	return metabase.types[name];
}

function findSymbol (metabase, name) {
	var symbol = metabase.symbols[name];

	/*
	 * TODO: current metabase has some constant without framework.
	 * it even has some common variable names like 'i', 'x', 'y' and 'result' so we want to exclude it here.
	 *
	 * "i": {
     *  "name": "i",
     *  "type": "int",
     ...
     * },
	 */
	if (symbol && (symbol.metatype == 'constant' || 
				  (symbol.metatype =='variable' && symbol.type.match(/^const|const$/)))
		) {
		return symbol;
	}
...
function findProperty (metabase, classname, property, isStatic) {
	// there's no static property in Objective-C
...
function findMethod (metabase, cls, method, args, isInstance, node, nodefail) {
	...
	// we need to search for *both* of class and protocols (e.g. NSObject has both class and protocol entry)
	if (!methods || !methods[method]) {
		cls = entry.superClass;
		entry = metabase.classes[cls];
		var entry_proto  = metabase.protocols[cls];
		while(entry || entry_proto) {
			if (entry && entry.methods && entry.methods[method]) {
				if (entry.methods[method]) {
					var result = _.filter(entry.methods[method], function(m){
						return isInstance == m.instance;
					});
					if (result) {
						methods = _.clone(entry.methods);
						break;
			...
			if (entry_proto && entry_proto.methods && entry_proto.methods[method]) {
				if (entry_proto.methods[method]) {
					var result = _.filter(entry_proto.methods[method], function(m){
						return isInstance == m.instance;
					});
					if (result) {
						methods = _.clone(entry_proto.methods);
						break;
					}
				}
			}
			cls = entry.superClass;
			entry = metabase.classes[cls];
			entry_proto = metabase.protocols[cls];
		}
	}
	...
	var result = _.filter(methods[method], function(m){
		return isInstance == m.instance && (m.hasVarArgs ? m.args.length <= argcount : m.args.length == argcount);
	});
	...
	if (result && result.length === 1) {
		if (/^alloc(WithZone)?$/.test(method)) {
			...
			result[0].returnTypeOrig = result[0].returnType;
...
function findConstructor (metabase, cls, args, node, nodefail) {
	...
	// We want to search up the type hierarchy for init* methods where the argcount matches
	// Then "new" should get translated to [[cls alloc] init*]
	while(entry) {
		var initMethodNames = _.filter(Object.keys(entry.methods), function(m){
			return /^init/.test(m);
		});
		// Ok we have all the method names that look like initializers
		// Now we need to actually look at the array of methods for each name and find a match based on argcount
		for (var c=0;c<initMethodNames.length;c++) {
			var name = initMethodNames[c];
			var result = _.filter(entry.methods[name], function(m){
				return m.instance && (m.hasVarArgs ? m.args.length <= argcount : m.args.length == argcount);
			});
			if (result && result.length === 1) {
				result = _.clone(result, true);
		...
		// didn't find a match, go up the hierarchy
		cls = entry.superClass;
		entry = metabase.classes[cls];
	}
	...
	Object.keys(symbols).forEach(function(key){
		...
		switch (entry.type) {
			...
			case 'method': {
				...
				else if (method.hasVarArgs) {
					if (entry.argcount < method.args.length) {
						nodefail(entry.location, "wrong number of arguments passed to "+entry.name.yellow+". expected variable number of arguments, with minimum of "+entry.argcount.yellow+" arguments.");
					}
				}
...
function defineMethod(options,state,arch,node,dict,fail) {
	...
	if (isConstructor) {
		...
		state.symbols[key] = {type:'method',metatype:'instance',symbolname:fn,instance:varname,returnType:methodObj.returnType,
					class:classname,name:methodname+signature,location:node.start,argcount:methodObj.args.length,
