...
function packageApp(options,args,callback) {
	...
	ioslib.xcode.frameworksFromDir(otherFrameworks, function(err, thirdPartyFrameworks) {
		...
		thirdPartyFrameworks.forEach(function(f){
			linkflags.push('-F' + path.resolve(f.directory, '..', '..'));
			linkflags.push('-framework ' + f.name);
		});

		ioslib.xcode.systemFrameworks(function(err,sysframeworks) {
