					...
					if (!uuids) {
						...
						if (options.device_id) {
							if (Array.isArray(options.device_id)) {
								log.fatal("more than one device connected. only one device can be connected at this time");
							}
							...
							if (uuids.indexOf(options.device_id)===-1) {
								...
								log.fatal("Can't find connected device id `"+options.device_id+"`");
							}
						}
						else {
							if (uuids.length > 1) {
