...
function executeDevice(name, build_dir, settings, callback, callback_logger, hide, device_id, quiet, unit, launch_timeout) {
	...
	function run () {
		...
		function stopLog() {
			if (!_stopped) {
				_stopped = true;
				log.info('------- '+('device log finished'.magenta.bold)+' -------');
			}
		}
		function finished(){
			if (!_finished) {
		...
		var sigkill = false,
			TI_MOCHA_UNIT = /TI_MOCHA_UNIT=(.*)/,
			AUTO_EXIT = /TI_EXIT/;

		function logger(label, buf) {
			lastLog = Date.now();
			buf && buf.split(/\n/).forEach(function(line){
				if (AUTO_EXIT.test(line)) {
					// skip it either way
					return unit && setTimeout(killIOSDeploy,10);
				}
				if (TI_MOCHA_UNIT.test(line)) {
					if (unit) {
						var results = TI_MOCHA_UNIT.exec(line)[1];
						results && log.info(line);
						setTimeout(killIOSDeploy,10);
...
function executeSimulator(name, build_dir, settings, callback, callback_logger, hide, unit, launch_timeout) {
	...
	function stopLog(){
		if (!_stopped) {
			_stopped = true;
			log.info('------- '+('simulator log finished'.magenta.bold)+' -------');
		}
	}
	...
	killSimulator(function(){
		util.downloadResourceIfNecessary('ios-sim', version, url, checksum, homeDir, function(err) {
			...
			var simulator = spawn(ios_sim, args),
				...
				TI_MOCHA_UNIT = /TI_MOCHA_UNIT=(.*)/,
				AUTO_EXIT = /TI_EXIT/; //hook to cause simulator to exit if this is in the log
			...
			function splitLogs(line, err) {
				...
				line.split(/\n/).forEach(function(content) {
					if (AUTO_EXIT.test(content)) {
						return unit && killSimulator(function(){
							timer && clearInterval(timer);
							finished();
						});
					}			
					if (TI_MOCHA_UNIT.test(content)) {
						if (unit) {
			...
			function logger(data, err) {
				...
				buf && !_stopped && splitLogs(buf, err);
			}
