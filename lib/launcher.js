...
function executeDevice(name, build_dir, settings, callback, callback_logger, hide, device_id, quiet, unit, launch_timeout) {
	...
	if (!fs.existsSync(ios_deploy)) {
		if (!process.stdout.isTTY) {
			log.fatal("please run `npm install ios-deploy` to install the required library for iOS device routines");
		}
		function install() {
			var dir = process.cwd(),
			...
			exec(cmd, function(err,stdout,stderr){
				...
				if (fs.existsSync(license)) {
					...
					log.fatal("Couldn't install ios-deploy library. Manually install with `sudo npm install ios-deploy -g`");
				}
				run();
			});
		}
	...
	killSimulator(function(){
		util.downloadResourceIfNecessary('ios-sim', version, url, checksum, homeDir, function(err) {
			...
			function splitLogs(line, err) {
				...
				if (TI_MOCHA_UNIT && TI_MOCHA_UNIT.test(line)) {
